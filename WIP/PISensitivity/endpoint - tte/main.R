...
###           Type of Endpoint: Time to Event (PFS)
################################################################################################
rm(list=ls())
setwd("~/Github Ripos/PSS/WIP/PIsensitivity/endpoint - tte")
library(data.table)
library(ggplot2)
library(survival)
library(survminer)
...
source('f_trueHR.r')                                  # Function to calculate the true causal effect(HR) for simulated datasets --> Unavailable for survival 
...
source('f_cutoff.r')                                  # Function to determine cutoff (pick any simulation set -> dat_in)
}
...
for (i in 1:nSim) {  
                     ...
                     inits        =  f_inits(dat=dat_in,seed,gamma_in,pm_result) 
