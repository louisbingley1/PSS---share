...
sim_data_cont <- function(n = 500, # sample size of simulated data
...
# Try one simulation data and one 
temp <- sim_data_cont(random_seed = 2)

# Check the potential value in the simulated data
mean(temp$Y1[temp$U == 1] - temp$Y0[temp$U == 1])# ss
mean(temp$Y1[temp$U == 2] - temp$Y0[temp$U == 2])# s1s1
mean(temp$Y1[temp$U == 3] - temp$Y0[temp$U == 3])# ss1
...
for(i in 1:200){
  print(i)
  temp <- sim_data_cont(random_seed = Seeds[i])
  res_temp <- PSPS_M_weighting(Z = temp$Z, 
                               D = temp$S, 
                               X = as.matrix(temp[, 1:5]), 
                               Y = temp$Y,
