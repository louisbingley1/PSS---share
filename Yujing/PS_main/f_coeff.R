...
f_coeff=function(AugData){
  ...
  coeff.1c=lm(Y ~ 1 + X_1 + X_2 + BASE, weights=w, data = data1c)$coefficients
  coeff.0c=lm(Y ~ 1 + X_1 + X_2 + BASE, weights=w, data = data0c)$coefficients
  coeff.1a=lm(Y ~ 1 + X_1 + X_2 + BASE, weights=w, data = data1a)$coefficients
  coeff.0a=lm(Y ~ 1 + X_1 + X_2 + BASE, weights=w, data = data0a)$coefficients
  coeff.1n=lm(Y ~ 1 + X_1 + X_2 + BASE, weights=w, data = data1n)$coefficients
  coeff.0n=lm(Y ~ 1 + X_1 + X_2 + BASE, weights=w, data = data0n)$coefficients
  coeff.1ca=lm(Y ~ 1 + X_1 + X_2 + BASE, weights=w, data = data1ca)$coefficients
  coeff.0ca=lm(Y ~ 1 + X_1 + X_2 + BASE, weights=w, data = data0ca)$coefficients
  ...
  coeff = cbind.data.frame(indexZU         = c('1c','0c','1a','0a','1n','0n'),
                           ...
                           coeff.BASE      = c(coeff.1c[4], coeff.0c[4], coeff.1a[4],coeff.0a[4], coeff.1n[4], coeff.0n[4]),
