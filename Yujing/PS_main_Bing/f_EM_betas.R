...
f_EM_betas=function(data,Z,D,X,N,V,USUBJID,beta.a = beta.a,beta.n=beta.n,iter.max,error0 ,Trace ){
  ...
  repeat{ if(Trace == T) {print(paste("The ", iter, "-th EM iteration!", sep=""))}
    ...
    # betas --> w_iter
    AugData    = f_augdata(data,Z,D,X,beta.a_old,beta.n_old,USUBJID) ; 
     
    # w_iter --> new betas
    augdata    = as.matrix(AugData[,1:6])
