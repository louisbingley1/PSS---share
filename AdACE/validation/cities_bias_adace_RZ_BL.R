...
po_mar       <- po_df %>%  dplyr::select(seed, subject, arm, timepoints, aval) %>%  rename(aval_mar = aval)
observed_mar <-  observed_df %>% left_join(po_mar, by=c("seed", "subject", "arm", "timepoints")) %>% rename(aval_mnar = aval)
...
head(data_out[[1]]$observed_df); tail(data_out[[10]]$observed_df)
head(observed_df);tail(observed_df)
head(data_out[[1]]$po_df); tail(data_out[[10]]$po_df)
head(po_df);tail(po_df)
#--------------------------------------------------------------------
...
sims              <- foreach (j            = 1:nsim,
                             ...
                             .init         = list(list(), list(), c(), c())) %dopar% {
                                    ...
                                    # Bing---------------------------------------------------------------------------------------
                                    data_ = data %>% mutate(DC = ifelse( dc_loe+dc_ee+dc_admin+dc_ae > 0 ,1 ,0), A=1-DC ) ; data_[1:20,]
                                    data_wide_ <- 
                                      data_ %>%   
                                      filter(seed ==j) %>%
                                      select(timepoints,A) %>%                    
