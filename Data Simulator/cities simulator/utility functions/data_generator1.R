...
data_generator1 = function (n_patient_vector, p_loe_max, z_l_loe, z_u_loe, p_ee_max, 
  ...
  #===================
  {
  if (length(unique(lengths(mean_list))) != 1) { warning("The lengths of means are not the same."); stop() }else{
    if (unique(lengths(mean_list)) != length(timepoints)) { warning("The length of timepoints need to be the as the length of means."); stop()  }
  }
  if (length(unique(lengths(sapply(mean_list, "[[", 1)))) !=  1) { warning("The first entry in the means are not all the same."); stop()}
  ...
  #===================
  {
  if (plot_po == TRUE) {    po_summary   = list()
                            for (i in 1:(length(mean_list))) {
                                            first_patient = max((n_patient_cumsum[i - 1] + 1),  1)                                  
    ...
    #----------------------------------------------
    {
              ...
              for (i in 1:length(mean_list)) { 
                ...
                # CHG from baseline, at each timepoint
                
                z_list[[i]]           = po_list[[i]][, -1] - po_list[[i]][,   1]
                
                # DC
                
                loe_list[[i]]         = p_loe_ee_function(z = z_list[[i]],   p_max = p_loe_max, z_l = z_l_loe, p_min = 0,  z_u = z_u_loe, up_good = (up_good == "Up"))
                ...
                ee_list[[i]]          = p_loe_ee_function(z = z_list[[i]],   p_max = p_ee_max, z_l = z_l_ee, p_min = 0, z_u = z_u_ee,  up_good = (up_good == "Down"))
    ...
    #----------------------------------------------
    {
              ...
              for (i in 1:(length(mean_list))) {
                ...
                if (length(delta_adjustment_in) == 0 | is.na(sum(delta_adjustment_in))) { delta_adjustment = numeric(length(mean_list))
                }else { delta_adjustment = delta_adjustment_in }
                delta_adj_current                       = current_group
                delta_adj_current[is.na(current_group)] = po_list[[i]][first_patient:n_patient_cumsum[i], 
                                                                       -1][is.na(current_group)] + abs(delta_adjustment[i]) *  (ifelse(up_good == "Up", -1, 1))
                if (up_good == "Up") { id_smaller_ref = (delta_adj_current < ir_current)  }else{
              ...
              dc_mean_list                = list(lapply(dc_loe_list, colMeans), lapply(dc_ee_list,   colMeans), lapply(dc_admin_list, colMeans), lapply(dc_ae_list,  colMeans), lapply(dc_final, colMeans))
   ...
   #----------------------------------------------
    {
              ...
              for (i in 1:length(mean_list)) {
                ...
                dc_df                      = dc_loe_temp_df %>% mutate(dc_ee = dc_ee_temp_df$dc_ee,  dc_admin = dc_admin_temp_df$dc_admin, dc_ae = dc_ae_temp_df$dc_ae)
                
                observed_temp_df           = simulated_data_output(n_patient_cumsum = n_patient_cumsum, 
                                                                   i = i, 
                                                                   first_patient = first_patient, 
                                                                   data_in = cbind(po_list[[i]][first_patient:n_patient_cumsum[i], 1],po_after_dc_list[[i]][first_patient:n_patient_cumsum[i],   ]),
                ...
                po_temp_df                 = simulated_data_output(n_patient_cumsum = n_patient_cumsum, 
                                                                   ...
                                                                   observed_indicator = observed_temp_df %>% distinct(seed, subject, arm) %>% mutate(observed = 1))
                
                
                ir_temp_df                 = simulated_data_output(n_patient_cumsum = n_patient_cumsum, 
                                                                   i = i, 
                                                                   first_patient = first_patient, 
                                                                   data_in = cbind(po_list[[i]][first_patient:n_patient_cumsum[i],  1], ir_data[[i]]), covariate_df = covariate_df, 
                                                                   timepoints = timepoints, 
                                                                   beta_list = beta_list, 
                                                                   seed_val = seed_val, 
                                                                   potential_outcomes = FALSE, 
                ...
                delta_adj_temp_df         = simulated_data_output(n_patient_cumsum = n_patient_cumsum, 
                                                                  i = i, 
                                                                  first_patient = first_patient, 
                                                                  data_in = cbind(po_list[[i]][first_patient:n_patient_cumsum[i],  1], delta_adj_data[[i]]), covariate_df = covariate_df, 
                                                                  timepoints = timepoints, 
                                                                  beta_list = beta_list, 
                                                                  seed_val = seed_val, 
                                                                  potential_outcomes = FALSE, 
                ...
                observed_df1  = observed_temp_df %>% left_join(dc_df,  by = c("seed", "subject", "arm", "timepoints"))
                observed_df   = rbind(observed_df, observed_df1)
                
                po_df1        = cbind(po_temp_df, dc_df[, -c(1:4)])
                po_df         = rbind(po_df, po_df1)  
                
                ir_df1        = ir_temp_df %>% left_join(dc_df, by = c("seed", "subject", "arm", "timepoints"))
                ir_df         = rbind(ir_df, ir_df1)
                
                delta_adj_df1 = delta_adj_temp_df %>% left_join(dc_df, by = c("seed", "subject", "arm", "timepoints"))
  ...
  if (length(delta_adjustment_in) == 0 | is.na(sum(delta_adjustment_in))) { # --> suppress 'Delta' output   
