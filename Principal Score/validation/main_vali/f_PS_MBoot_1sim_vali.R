f_PS_MBoot_1sim_vali = function(data_boot, ep0,ep1,M,iter.max,error0,seed_M_v){ 

  #  Bootstrap -> to compute se(ACE) -- the sd of all bootstrap ACEs, within 1 simulation;
 
  N            = nrow(data_boot)                     # number of subjects in data
  ...
  for(m in 1:M){ # m=1
    ...
    boot            = data_boot[sample(1:N,n,replace=T),] ;head(boot) # re-sample data with replacement
    boot            = as.data.frame(boot)
    result          = f_PS_vali( data <- boot, ep1,ep0,beta.a,beta.n, iter.max, error0)
