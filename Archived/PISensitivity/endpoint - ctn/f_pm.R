...
f_pm = function(dat){
    ...
    beta0                 = glm( Y0_ ~ S1 + X1 + X2,  data = dat  )$coefficient[1]
    beta1                 = glm( Y0_ ~ S1 + X1 + X2,  data = dat  )$coefficient[3]
    beta2                 = glm( Y0_ ~ S1 + X1 + X2,  data = dat  )$coefficient[4]
    beta0_plus_delta      = glm( Y1  ~ S1 + X1 + X2,  data = dat  )$coefficient[1]
    eta                   = glm( Y1  ~ S1 + X1 + X2,  data = dat  )$coefficient[2]
